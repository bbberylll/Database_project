...
public class PassengerDAO {
 ...
 // ================================================
    //추가: Connection을 외부에서 받는 오버로드 메서드
    // ================================================

    /** *신규* – 서비스 계층에서 트랜잭션을 관리할 때 사용 */
    public void insertPassenger(Connection conn, Passenger p) {
        String sql = "INSERT INTO passenger (passenger_id, first_name, last_name, email, phone_number) VALUES (?, ?, ?, ?, ?)";
        ...
        try {
            ...
            throw new RuntimeException("Passenger insert failed (트랜잭션 공유): " + e.getMessage(), e);
        } finally {
            DBUtil.closeQuietly(pstmt);
            // conn은 service가 관리하므로 여기서 닫지 않습니다.
        }
    }

    /** *신규* – 서비스 계층에서 트랜잭션을 관리할 때 사용 */
    public void updatePassengerEmail(Connection conn, String passengerId, String newEmail) {
        String sql = "UPDATE passenger SET email = ? WHERE passenger_id = ?";
        PreparedStatement pstmt = null;
        try {
            pstmt = conn.prepareStatement(sql);
            ...
            throw new RuntimeException("Passenger email update failed (트랜잭션 공유): " + e.getMessage(), e);
        } finally {
            DBUtil.closeQuietly(pstmt);
        }
    }
    ...
    /** *신규* – 서비스 계층에서 트랜잭션을 관리할 때 사용 */
    public void deletePassengerById(Connection conn, String passengerId) {
        String sql = "DELETE FROM passenger WHERE passenger_id = ?";
        PreparedStatement pstmt = null;
        try {
            pstmt = conn.prepareStatement(sql);
            ...
            throw new RuntimeException("Passenger delete failed (트랜잭션 공유): " + e.getMessage(), e);
        } finally {
            DBUtil.closeQuietly(pstmt);
        }
    }
